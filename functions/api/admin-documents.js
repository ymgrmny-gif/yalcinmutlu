...
export async function onRequestPost({ request }) {
  ...
  const contentLength = Number(request.headers.get('Content-Length') || '0');
  if (contentLength > 32_000) return json(413, { ok: false, code: 'PAYLOAD_TOO_LARGE' });
  ...
  if (action === 'login') {
    if (isRateLimited(request, 'admin-login', 10, 15 * 60 * 1000)) {
  ...
  if (['createUser', 'toggleUser', 'resetUserPassword', 'setPermission'].includes(action)) {
    return json(410, { ok: false, code: 'LEGACY_ACCESS_RETIRED' });
  }

  const allowed = new Set(['session', 'dashboard', 'changePassword', 'toggleDocument', 'documentUrl', 'deleteDocument', 'logout']);
